...
class MrClay_CookieStorage {
    ...
    //DEPRECATED uses ECB mode
    public static function encryptECB($key, $str)
    {
    ...
    //DEPRECATED uses ECB mode
    public static function decryptECB($key, $data)
    {
    ...
    public function getDefaults()
    {
        return array(
            ...
            ,'encryptFunc' => array('MrClay_CookieStorage', 'encryptECB')
            ,'decryptFunc' => array('MrClay_CookieStorage', 'decryptECB')
            ,'hashAlgo' => 'ripemd160'
    ...
    private function _storeEncrypted($name, $str)
    {
		...
		//$cryptKey = COOKIE_STORAGE_ENCRYPTION_KEY;
	...
	//get raw data of encrypted instead of storing to cookie
	public function encryptRaw($name, $str){
		if (! is_callable($this->_o['encryptFunc'])) {
            $this->errors[] = 'Encrypt function not callable';
            return false;
        }
		...
		//return raw data instead of storing to cookie
		return $raw;
	}

	//decrypts a raw cookie value
	public function decryptRaw($name,$raw)
    {
        if (isset($this->_returns[self::MODE_ENCRYPT][$name])) {
            return $this->_returns[self::MODE_ENCRYPT][$name][0];
        }
